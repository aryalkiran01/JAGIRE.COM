...
export const Route = createFileRoute("/_authenticated/employer/jobs/$jobId")({ component: JobApplicants });
...
function JobApplicants() {
  ...
  const { data: job } = useQuery({ queryKey: ["employer-job", jobId], queryFn: async () => (await supabase.from("jobs").select("*").eq("id", jobId).maybeSingle()).data });
  const { data: apps } = useQuery({
    queryKey: ["job-apps", jobId],
    queryFn: async () => (await supabase.from("applications").select("*, profile:profiles!applications_applicant_id_fkey(id, full_name, email, headline, avatar_url), resume:resumes(id, file_name, file_path)").eq("job_id", jobId).order("applied_at", { ascending: false })).data ?? [],
  });
  const update = useMutation({
    ...
    onSuccess: () => { toast.success("Updated"); qc.invalidateQueries({ queryKey: ["job-apps"] }); },
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-3">
        {apps?.map((a: any) => (
          <ApplicantCard key={a.id} a={a} onStatus={(v) => update.mutate({ id: a.id, status: v })} />
        ))}
        {!apps?.length && <Card><CardContent className="p-12 text-center text-muted-foreground">No applications yet.</CardContent></Card>}
      </div>
    </div>
  );
}
...
function ApplicantCard({ a, onStatus }: { a: any; onStatus: (v: string) => void }) {
  ...
  async function openResume() {
    ...
    const { data, error } = await supabase.storage.from("resumes").createSignedUrl(a.resume.file_path, 60);
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <Badge className={a.status === "shortlisted" ? "gradient-brand text-primary-foreground" : ""}>{a.status}</Badge>
          ...
          <Collapsible open={open} onOpenChange={setOpen} className="mt-3">
            <CollapsibleTrigger asChild>
              <Button variant="ghost" size="sm" className="text-xs">
                <ChevronDown className={`mr-1 h-3 w-3 transition-transform ${open ? "rotate-180" : ""}`} />
