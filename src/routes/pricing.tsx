    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-16 max-w-5xl">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { name: "Free", price: "$0", features: ["Unlimited job search", "AI resume scan", "5 applications/day", "Basic career tips"], cta: "Get started", featured: false },
            { name: "Employer Starter", price: "$49/mo", features: ["3 active job posts", "AI candidate ranking", "Applicant management", "Email support"], cta: "Start hiring", featured: true },
            { name: "Enterprise", price: "Custom", features: ["Unlimited job posts", "Priority AI matching", "Google Meet integration", "Dedicated support"], cta: "Contact sales", featured: false },
          ].map((p) => (
            <Card key={p.name} className={p.featured ? "border-primary shadow-glow" : ""}><CardContent className="p-6">
              ...
              <Button className={`w-full ${p.featured ? "gradient-brand text-primary-foreground" : ""}`} variant={p.featured ? "default" : "outline"}>{p.cta}</Button>
            </CardContent></Card>
          ))}
        </div>
      </div>
