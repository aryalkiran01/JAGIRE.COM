...
export const Route = createFileRoute("/_authenticated/resume-scanner")({ component: ResumeScanner });
...
function ResumeScanner() {
  ...
  const [matches, setMatches] = useState<Array<{ id: string; title: string; company: string | null; score: number }>>([]);

  const { data: resume } = useQuery({
    queryKey: ["my-resume-full", user?.id],
    enabled: !!user,
    queryFn: async () =>
      (await supabase.from("resumes").select("*").eq("user_id", user!.id).eq("is_default", true).maybeSingle()).data,
  });
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const ins = await supabase.from("resumes").insert({
        ...
        file_size: file.size, mime_type: file.type, is_default: true,
      }).select().single();
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl space-y-6">
      ...
      <Card><CardContent className="p-6">
        <label className="flex items-center gap-3 cursor-pointer border-2 border-dashed rounded-lg p-6 hover:bg-muted transition">
          ...
          <div>
            <div className="font-medium">{resume ? resume.file_name : "Upload resume (PDF or DOCX)"}</div>
            <div className="text-xs text-muted-foreground">
              {busy ? "Extracting text and scoring with AI…" : `Click to ${resume ? "replace and re-scan" : "upload — we'll auto-scan it"}`}
            </div>
          </div>
          <input type="file" className="hidden" accept=".pdf,.docx" onChange={handleFile} disabled={busy} />
        </label>
        {resume && (
          <div className="mt-3 flex justify-end">
            <Button size="sm" variant="outline" onClick={reAnalyze} disabled={busy}>
              {busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}Re-analyze
            </Button>
          </div>
        )}
      </CardContent></Card>
      ...
      {resume?.overall_score != null && (
        <Card><CardContent className="p-6 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {scores.map((s) => (
              <div key={s.label}>
                <div className="flex justify-between text-sm mb-1"><span>{s.label}</span><span className="font-semibold">{s.value ?? 0}/100</span></div>
      ...
      {matches.length > 0 && (
        <Card><CardContent className="p-6 space-y-3">
          <h2 className="text-xl font-bold flex items-center gap-2"><Briefcase className="h-5 w-5" /> Top job matches</h2>
          <p className="text-sm text-muted-foreground">Based on your resume skills.</p>
          <div className="space-y-2">
            {matches.map((m) => (
              <Link key={m.id} to="/jobs/$jobId" params={{ jobId: m.id }} className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted transition">
                <div>
