...
function CompanyDetail() {
  ...
  const { data: company } = useQuery({
    ...
    queryFn: async () => (await supabase.from("companies").select("*").eq("slug", slug).maybeSingle()).data,
  });
  const { data: jobs } = useQuery({
    queryKey: ["company-jobs", company?.id],
    enabled: !!company?.id,
    queryFn: async () => (await supabase.from("jobs").select("*").eq("company_id", company!.id).eq("status", "active")).data ?? [],
  });
  const { data: reviews } = useQuery({
    queryKey: ["company-reviews", company?.id],
    enabled: !!company?.id,
    queryFn: async () => (await supabase.from("reviews").select("*, reviewer:profiles!reviews_reviewer_id_fkey(full_name, avatar_url), replies:review_replies(*)" as any).eq("company_id", company!.id).order("created_at", { ascending: false })).data ?? [],
  });
  ...
  async function submitReply(reviewId: string) {
    ...
    if (!text) return;
    const { error } = await supabase.from("review_replies" as any).insert({ review_id: reviewId, author_id: user!.id, content: text });
  ...
  async function submitReview() {
    ...
    const { error } = await supabase.from("reviews").insert({
      company_id: company.id, reviewer_id: user.id, rating, title: title || null, content: content || null,
    });
    if (error) return toast.error(error.message);
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl">
          ...
          <CardContent className="p-8">
            <div className="flex items-start gap-6">
              <div className="h-24 w-24 rounded-xl bg-muted flex items-center justify-center overflow-hidden">
                {company.logo_url ? <img src={company.logo_url} alt="" className="h-full w-full object-cover" /> : <Building2 className="h-12 w-12" />}
              </div>
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  ...
                  {company.headquarters && <Badge variant="secondary"><MapPin className="mr-1 h-3 w-3" />{company.headquarters}</Badge>}
            ...
            {(company as any).verified && <Badge className="mt-3 gradient-brand text-primary-foreground">✓ Verified</Badge>}
            <div className="grid md:grid-cols-2 gap-4 mt-4 text-sm">
              {(company as any).mission && <div><div className="font-semibold">Mission</div><p className="text-muted-foreground">{(company as any).mission}</p></div>}
              {(company as any).vision && <div><div className="font-semibold">Vision</div><p className="text-muted-foreground">{(company as any).vision}</p></div>}
              {(company as any).culture && <div><div className="font-semibold">Culture</div><p className="text-muted-foreground">{(company as any).culture}</p></div>}
              {(company as any).hiring_process && <div><div className="font-semibold">Hiring process</div><p className="text-muted-foreground">{(company as any).hiring_process}</p></div>}
            </div>
            {((company as any).benefits?.length ?? 0) > 0 && <div className="mt-4"><div className="font-semibold text-sm mb-1">Benefits</div><div className="flex flex-wrap gap-1">{(company as any).benefits.map((b: string) => <Badge key={b} variant="secondary">{b}</Badge>)}</div></div>}
            {((company as any).technologies?.length ?? 0) > 0 && <div className="mt-3"><div className="font-semibold text-sm mb-1">Technologies</div><div className="flex flex-wrap gap-1">{(company as any).technologies.map((b: string) => <Badge key={b} variant="outline">{b}</Badge>)}</div></div>}
          </CardContent>
        </Card>
        ...
        {user && (
          <Card className="mb-4"><CardContent className="p-6 space-y-3">
            ...
            <Textarea rows={3} placeholder="Share your experience…" value={content} onChange={(e) => setContent(e.target.value)} />
            <Button onClick={submitReview} className="gradient-brand text-primary-foreground">Post review</Button>
          </CardContent></Card>
        )}
            ...
            <Card key={r.id}><CardContent className="p-4">
              ...
              {isOwner && !(r.replies?.length) && (
                <div className="mt-3 flex gap-2">
                  <Input placeholder="Reply as company…" value={replyDrafts[r.id] ?? ""} onChange={(e) => setReplyDrafts((d) => ({ ...d, [r.id]: e.target.value }))} />
                  <Button size="sm" onClick={() => submitReply(r.id)}>Reply</Button>
                </div>
              )}
            </CardContent></Card>
          ))}
