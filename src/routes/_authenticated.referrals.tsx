...
function Referrals() {
  ...
  async function invite() {
    ...
    const { error } = await supabase.from("referrals").insert({
      referrer_id: user.id, referred_email: email, code: profile.referral_code, status: "pending",
    });
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <h1 className="text-3xl font-bold flex items-center gap-2 mb-6"><Gift className="h-7 w-7" />Refer & earn</h1>

      <Card className="mb-6"><CardHeader><CardTitle>Your referral link</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">Share this link. When someone signs up through it, you earn credits.</p>
        </CardContent>
      </Card>

      <Card className="mb-6"><CardHeader><CardTitle>Invite by email</CardTitle></CardHeader>
        <CardContent className="flex gap-2">
          <Input type="email" placeholder="friend@example.com" value={email} onChange={(e) => setEmail(e.target.value)} />
          <Button onClick={invite}><Send className="h-4 w-4 mr-1" />Invite</Button>
        </CardContent>
      </Card>

      <Card><CardHeader><CardTitle>Your referrals</CardTitle></CardHeader>
          ...
          {referrals?.map((r) => (
            <div key={r.id} className="flex items-center justify-between p-3 border rounded">
              ...
              <div className="flex items-center gap-2">
                ...
                {r.reward_credits > 0 && <Badge className="gradient-brand text-primary-foreground">+{r.reward_credits}</Badge>}
              </div>
            </div>
          ))}
        </CardContent>
