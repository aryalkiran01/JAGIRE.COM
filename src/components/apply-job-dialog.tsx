...
export function ApplyJobDialog({ jobId, disabled, applied, deadlinePassed, jobClosed }: { jobId: string; disabled?: boolean; applied?: boolean; deadlinePassed?: boolean; jobClosed?: boolean }) {
  const { user } = useAuth();
  ...
  function friendlyError(e: any): string {
    ...
    const code = e?.code;
    if (code === "23505" || /duplicate key/i.test(msg)) return "You've already applied to this job.";
    if (/cannot apply to a job posted by your own company/i.test(msg)) return "You can't apply to a job posted by your own company.";
    if (/row-level security|permission denied/i.test(msg)) return "You don't have permission to apply. Try signing out and back in.";
    if (/violates check constraint|null value in column/i.test(msg)) return "Some required application details are missing.";
    if (/Object not found|not_found/i.test(msg)) return "Resume upload failed. Please try a different file.";
  ...
  const blockedLabel = applied ? "Already applied" : jobClosed ? "Job closed" : deadlinePassed ? "Deadline passed" : null;

  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button disabled={disabled || !!blockedLabel} className="w-full gradient-brand text-primary-foreground">
          {blockedLabel ?? "Apply now"}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>{success ? "Application submitted" : "Apply to this job"}</DialogTitle></DialogHeader>
        {success ? (
          <div className="py-6 flex flex-col items-center text-center gap-3">
            <CheckCircle2 className="h-14 w-14 text-green-500" />
            <p className="text-sm text-muted-foreground">Your application is on its way to the employer. You can track its status any time.</p>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {file && <p className="text-xs text-muted-foreground mt-1">Selected: {file.name} ({Math.round(file.size / 1024)} KB)</p>}
          </div>
          <div>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={submitting || (!file && !resumes?.length)} className="gradient-brand text-primary-foreground">
